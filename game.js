...
let gameState = 'mainmenu'; // 'mainmenu', 'playing', 'upgrade', 'gameover', 'upgrades', 'run_summary'
let upgradeOptions = [];
...
function spawnEnemy() {
  ...
  if (gameTime > 180 && random() < 0.2) { // After 3 minutes, 20% chance of stalker enemy
    enemyType = 'stalker';
  }
...
function killEnemy(index) {
  ...
  if (e.type === 'boss') {
    ...
    musicManager.playTrack('intense');
...
function selectUpgrade(index) {
  if (index >= 0 && index < upgradeOptions.length) {
    ...
    trackUpgrade(selectedUpgrade.name, gameTime);
...
function drawGameOverScreen() {
  ...
  // View Run Summary button
    let summaryButton = createButton('View Run Summary');
    summaryButton.position(width / 2 - 100, height * 3 / 4 + 40);
    summaryButton.size(200, 50);
    summaryButton.mousePressed(() => {
...
class Enemy {
  constructor(x, y, type, level) {
    ...
    switch (type) {
      ...
      case 'stalker':
        ...
        this.health = 50 + level * 6;
        this.speed = random(1.5, 2.2);
        this.color = color(100, 255, 100);
        this.damage = 12;
        this.isCloaked = false;
        this.cloakDuration = 180; // 3 seconds
        this.cloakCooldown = 300; // 5 seconds
        this.lastCloak = 0;
        break;
      case 'boss':
        this.size = 80;
  ...
  update(player) {
    ...
    if (this.type === 'stalker') {
      if (millis() - this.lastCloak > this.cloakCooldown) {
        this.isCloaked = true;
        this.lastCloak = millis();
      }
      if (this.isCloaked && millis() - this.lastCloak > this.cloakDuration) {
        this.isCloaked = false;
      }
    }
  }
  ...
  draw() {
    ...
    if (this.type === 'stalker') {
      let alpha = this.isCloaked ? 50 : 255;
